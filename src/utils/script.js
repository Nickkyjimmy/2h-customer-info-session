...
lenis.on("scroll", ({scroll }) => {
    ...
    scrollProgress = Math.min((1, scroll / maxScroll) * CONFIG.speed, 1.1);
});
...
ScrollTrigger.create({
    ...
    onUpdate: (self) => {
        ...
        words.forEach((word, index) => {
            ...
            const nextWordsProgress = (index + 1) / totalWords;
            
            let opacity = 0;
            
            if (progress >= nextWordsProgress){
                opacity = 1;
            } else if (progress >= wordProgress){
                opacity = (progress - wordProgress) / (nextWordsProgress - wordProgress);
            }
