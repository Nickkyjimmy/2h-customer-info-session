...
export default function ShuffleCardPage() {
  ...
  useEffect(() => {
    ...
    const createItem = () => {
      ...
      for (let i = 0; i < ITEMS_COUNT; i++) {
        ...
        img.src = cardData[cardIndex - 1]?.image || `https://static.momocdn.net/app/img/cio_website/chrismas-shuffle/${cardIndex}.jpg`;
        img.alt = `Image ${i}`;
    ...
    const initAnimation2 = () => {
      ...
      for (let i = 0; i < config.imageCount; i++) {
        ...
        const img = document.createElement("img");
        img.src = cardData[cardIndex]?.image || `https://static.momocdn.net/app/img/cio_website/chrismas-shuffle/${cardIndex + 1}.jpg`;
        ...
        gsap.to(gallery2, {
          ...
          onComplete: () => {
            ...
            if (selectedCard) {
              ...
              // Calculate title position (bottom: 25%)
              const titleBottom = window.innerHeight * 0.25;
              const titleY = window.innerHeight - titleBottom;
              
              // Calculate offsets
              const offsetX = viewportCenterX - cardCenterX;
              const offsetY = titleY - cardCenterY - 50; // Position card above title
              
              // Apply fine-tuning adjustments to position card above name
              if (Math.abs(offsetX) > 1 || Math.abs(offsetY) > 1) {
                const currentX = gsap.getProperty(gallery2, "x") as number || 0;
                const currentY = gsap.getProperty(gallery2, "y") as number || 0;
                
                gsap.to(gallery2, {
                  x: currentX + offsetX / 5,
                  y: currentY + offsetY / 5,
                  duration: 0.3,
                  ease: "power2.out",
                  onComplete: () => {
      ...
      skipHideActionRef.current = skipHideAction;
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-[#e3e3db] text-[#1f1f1f] overflow-hidden">
      ...
      <div
        ref={container1Ref}
        className="container animation1-container"
      >
      ...
      <div
        ref={container2Ref}
        className="container animation2-container"
      ...
      <footer>
        <div className="footer-container">
          <div className="footer-content">
            <p>© 2025 Shuffle Card. All rights reserved.</p>
          </div>
        </div>
      </footer>
